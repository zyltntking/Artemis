...
public class ArtemisAuthenticationHandler : AuthenticationHandler<ArtemisAuthenticationOptions>
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        {
            return AuthenticateResult.Fail("空令牌");
        }

        // 若存在定义的模式，则移除
        if (authorizationToken.StartsWith(AuthorizationOptions.RequestHeaderTokenSchema, StringComparison.OrdinalIgnoreCase))
        {
    ...
    private async Task<AuthenticateResult> ValidateToken(string token)
    {
        ...
        var document = await Cache.FetchTokenDocumentAsync<TokenDocument>(cacheTokenKey, cancellationToken: Context.RequestAborted);
        ...
        if (document != null)
        {
            ...
            {
                new(Data.Shared.Transfer.ClaimTypes.Authorization, token),
                new(Data.Shared.Transfer.ClaimTypes.UserId, document.UserId.IdToString() ?? string.Empty),
                new(Data.Shared.Transfer.ClaimTypes.UserName, document.UserName),
                new(Data.Shared.Transfer.ClaimTypes.EndType, document.EndType)
            };

            var roles = document.Roles
                .Select(item => new Claim(Data.Shared.Transfer.ClaimTypes.Role, item.Name));
            user.AddRange(roles);
            ...
            user.Add(new(Data.Shared.Transfer.ClaimTypes.MateActionName, actionName));

            user.Add(new(Data.Shared.Transfer.ClaimTypes.MateRoutePath, routePath));
