...
///     Artemis凭据处理器
/// </summary>
internal class ArtemisAuthorizationHandlerStandOnly : AuthorizationHandler<IArtemisAuthorizationRequirement>
{
    /// <summary>
    ///     构造
    /// </summary>
    ...
    public ArtemisAuthorizationHandlerStandOnly(
        IDistributedCache cache,
        IHttpContextAccessor httpContextAccessor,
        IOptions<ArtemisAuthorizationOptions> options,
        ILogger<ArtemisAuthorizationHandlerStandOnly> logger)
    {
        Cache = cache;
        ...
        if (requirement is TokenRequirement)
        {
            if (httpContext is not null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(tokenSymbol))
                {
                    ...
                    {
                        httpContext.CacheTokenDocument(Options.ContextItemTokenKey, document);
        ...
        ref string message)
    {
            ...
            (document.UserClaims.Any() || document.RoleClaims.Any()))
        {
            ...
            var requireClaimStamps =
                claimRequirement.Claims.Select(Normalize.KeyValuePairStamp);
