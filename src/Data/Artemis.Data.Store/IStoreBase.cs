...
public interface IStoreBase<in TEntity,TKey> : IDisposable where TEntity : IModelBase<TKey> where TKey : IEquatable<TKey>
{
    ...
    /// <returns>Id</returns>
    Task<TKey> GetIdAsync(TEntity entity, CancellationToken cancellationToken = default);
    ...
    /// <returns>Id字符串</returns>
    Task<string> GetIdStringAsync(TEntity entity, CancellationToken cancellationToken = default);
    ...
    /// <returns>判断结果</returns>
    Task<bool> IsDeletedAsync(TEntity entity, CancellationToken cancellationToken = default);
