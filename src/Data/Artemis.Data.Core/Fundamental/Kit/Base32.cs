...
public static class Base32
{
    ...
    public static string GenerateBase32()
    {
        ...
        return string.Create((length + 4) / 5 * 8, 0, static (buffer, _) =>
        {
            ...
            {
                var numCharsToOutput = GetNextGroup(bytes, ref offset, out var a, out var b, out var c, out var d,
                    out var e, out var f, out var g, out var h);
    ...
    public static string ToBase32(byte[] input)
    {
        ...
        {
            var numCharsToOutput = GetNextGroup(input, ref offset, out var a, out var b, out var c, out var d,
                out var e, out var f, out var g, out var h);
