...
public class RemoveDuplicateApiVersionParameterFilter : IOperationFilter
{
    ...
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        var isApiVersionParameterDuplicated = operation.Parameters.Count(parameter => parameter.Name == HttpHeaderNamesApiVersion) > 1;
        if (!isApiVersionParameterDuplicated)
        {
            return;
        }

        var description = apiDescription.ParameterDescriptions.FirstOrDefault(p => p is { Name: HttpHeaderNamesApiVersion, DefaultValue: not null });
