...
///     认证用户管理
/// </summary>
public sealed class IdentityUserManager : Manager, IIdentityUserManager
{
    /// <summary>
    ///     创建新的管理器实例
    /// </summary>
    ...
    public IdentityUserManager(
        ...
        IManagerOptions? options = null) : base(options)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (userExists)
            return await UserStore.DeleteAsync(id, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var idList = await UserStore
            .KeyMatchQuery(ids)
            .Select(user => user.Id)
            .ToListAsync(cancellationToken);

        if (idList.Any())
            return await UserStore.DeleteAsync(idList, cancellationToken);
