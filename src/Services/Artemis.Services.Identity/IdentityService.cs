...
public class IdentityService<TUser> : IdentityService<TUser, Guid>, IIdentityService<TUser> where TUser : IdentityUser<Guid>
{
    /// <summary>
    /// 认证服务构造
    /// </summary>
...
public abstract class IdentityService<TUser, TKey> where TUser : IdentityUser<TKey> where TKey : IEquatable<TKey>
{
    ...
    /// <returns></returns>
    public async Task<AttachResult<TUser>> SignUp(string username, string password, string? email = null, string? phone = null, CancellationToken cancellationToken = default)
    {
